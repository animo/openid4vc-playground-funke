...
export async function createOrUpdateVerifier(options: PlaygroundVerifierOptions) {
  if (await doesVerifierExist(options.verifierId)) {
    await agent.modules.oid4vc.verifier.updateVerifierMetadata({
      verifierId: options.verifierId,
      clientMetadata: options.clientMetadata,
    })
  } else {
    return agent.modules.oid4vc.verifier.createVerifier({
      clientMetadata: options.clientMetadata,
      verifierId: options.verifierId,
    })
  }
...
export async function doesVerifierExist(verifierId: string) {
  try {
    await agent.modules.oid4vc.verifier.getVerifierByVerifierId(verifierId)
    return true
  } catch (error) {
    return false
  }
...
export async function getVerifier(verifierId: string) {
  return agent.modules.oid4vc.verifier.getVerifierByVerifierId(verifierId)
}
